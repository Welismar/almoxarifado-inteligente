  ...
  return (
    <main className="materials-page">
      ...
      <section className="materials-grid">
        <article className="panel material-form-panel"><div className="panel-heading"><div><h2>Novo material</h2><p>Informações básicas do item</p></div></div><form className="material-form" onSubmit={handleSubmit}><label>Código interno<input value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} placeholder="MAT-0001" required /></label><label>Descrição<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Ex.: Cimento CP-II 50kg" required /></label><div className="form-row"><label>Unidade<select value={form.unit} onChange={(event) => setForm({ ...form, unit: event.target.value })}><option>UN</option><option>CX</option><option>PC</option><option>JG</option><option>KIT</option><option>PAR</option><option>KG</option><option>G</option><option>T</option><option>M</option><option>CM</option><option>MM</option><option>M2</option><option>M3</option><option>L</option><option>ML</option><option>SC</option><option>FD</option><option>RL</option><option>GL</option><option>TON</option><option>HR</option><option>DIA</option><option>MES</option></select></label><label>Estoque mínimo<input type="number" min="0" step="0.001" value={form.minimumStock} onChange={(event) => setForm({ ...form, minimumStock: event.target.value })} /></label></div><label>Custo médio<input type="number" min="0" step="0.01" value={form.averageCost} onChange={(event) => setForm({ ...form, averageCost: event.target.value })} /></label>{error && <p className="login-error" role="alert">{error}</p>}{message && <p className="success-message" role="status">{message}</p>}<button className="primary-button" type="submit" disabled={isSaving}>{isSaving ? "Salvando..." : "Cadastrar material"}</button></form></article>
