...
export async function GET(request: Request) {
  ...
  const [materialResponse, movementsResponse] = await Promise.all([
    ...
    fetch(`${url}/rest/v1/stock_movements?select=id,type,quantity,unit_cost,lot,created_at,responsible_id&material_id=eq.${encodeURIComponent(materialId)}${params.get("projectId") ? `&project_id=eq.${encodeURIComponent(params.get("projectId")!)}` : ""}${params.get("lot") ? `&lot=eq.${encodeURIComponent(params.get("lot")!)}` : ""}${params.get("from") ? `&created_at=gte.${encodeURIComponent(`${params.get("from")}T00:00:00.000Z`)}` : ""}${params.get("to") ? `&created_at=lt.${encodeURIComponent(`${params.get("to")}T23:59:59.999Z`)}` : ""}${params.get("type") ? `&type=eq.${encodeURIComponent(params.get("type")!)}` : ""}&order=created_at.asc`, { headers, cache: "no-store" }),
  ]);
