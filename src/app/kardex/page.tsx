...
export default function KardexPage() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { fetch("/api/movement-options", { cache: "no-store" }).then((response) => response.json()).then((result: { materials?: Material[]; projects?: Project[] }) => { setMaterials(result.materials ?? []); setProjects(result.projects ?? []); }).catch(() => setError("Não foi possível carregar os materiais.")); }, 0); return () => window.clearTimeout(timer); }, []);
  ...
  const rows = movements.reduce<Array<Movement & { balance: number }>>((result, movement) => {
      ...
      return [...result, { ...movement, balance: previousBalance + (incoming ? Number(movement.quantity) : -Number(movement.quantity)) }];
    }, []).reverse();
