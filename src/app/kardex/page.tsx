...
export default function KardexPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      fetch("/api/movement-options", { cache: "no-store" })
        ...
        .then((result: { materials?: Material[]; projects?: Project[] }) => {
  ...
  async function loadKardex(id = materialId) {
    ...
    if (!response.ok)
      setError(result.error ?? "Não foi possível carregar o Kardex.");
    else {
  ...
  const rows = movements
    .reduce<Array<Movement & { balance: number }>>((result, movement) => {
      ...
      return [
        ...result,
        {
          ...
          balance:
            previousBalance +
            (incoming ? Number(movement.quantity) : -Number(movement.quantity)),
        },
      ];
    }, [])
    .reverse();
    ...
    <main className="materials-page">
      <header className="materials-header">
        <div>
          ...
          <h1>Kardex do estoque</h1>
          <p className="subtitle">
            Histórico completo de movimentações por material.
          </p>
        </div>
        <div className="top-avatar">MC</div>
      </header>
      <section className="panel kardex-selector">
        <label>
          Material
          <select
            value={materialId}
            onChange={(event) => void loadKardex(event.target.value)}
          >
        ...
        <div className="kardex-filters">
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setFilters({ ...filters, projectId: event.target.value })
              }
            >
          ...
          <label>
            ...
            <input
              value={filters.lot}
              onChange={(event) =>
                setFilters({ ...filters, lot: event.target.value })
              }
              placeholder="Todos"
            />
          </label>
      ...
      {error && (
        <p className="login-error kardex-message" role="alert">
          {error}
        </p>
      )}
      {material && (
        <section className="kardex-summary">
          <div>
            <span>Material</span>
          ...
          <div>
            ...
            <small>Registros imutáveis</small>
          </div>
        </section>
      )}
      {loading ? (
        ...
        material && (
          <section className="panel kardex-panel">
            <div className="panel-heading">
              <div>
                <h2>Histórico de movimentações</h2>
                <p>Do registro mais recente ao mais antigo</p>
              </div>
              <button
                className="select-button"
                type="button"
                    ...
                    rows.map((movement) => {
                      ...
                      return (
                        <tr key={movement.id}>
                          ...
                          <td>
                            <span
                              className={`movement-tag ${incoming ? "in" : "out"}`}
                            >
                              {labels[movement.type] ?? movement.type}
                            </span>
                          </td>
                          ...
                          <td>
                            <strong>{movement.balance}</strong>
                          </td>
                          <td>R$ {Number(movement.unit_cost).toFixed(2)}</td>
                          <td className="muted">{movement.lot || "-"}</td>
                        </tr>
                      );
                    })
                  )}
