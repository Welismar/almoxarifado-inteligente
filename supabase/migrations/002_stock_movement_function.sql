create policy "company members can insert materials" on public.materials
...
begin
  ...
  if not exists (
    ...
    where id = p_material_id and company_id = v_company_id and status = 'active'
  ) then
  ...
  values (
    ...
    jsonb_build_object('type', p_type, 'quantity', p_quantity, 'material_id', p_material_id)
  );
...
grant execute on function public.record_stock_movement(uuid, uuid, uuid, uuid, public.movement_type, numeric, numeric, text, date, text, uuid, text) to authenticated;
